...
def main(top_p: float, temperature: float, choose, prompt_text: str, model: str):
    if model == 'Music Recommender (full)':
                ...
                with markdown_placeholder:
                    with st.spinner(f'Calling tools ...'):
                        ...
                        for tool_call in tool_calls:
                            ...
                            function_to_call = available_functions[function_name]
                            function_args = json.loads(tool_call.function.arguments)
                            if function_name == "Online_Music_Searcher":
                                function_response = function_to_call(
    ...
    elif model == 'Music Recommender (without local knowledge base)':
        if 'tool_history' not in st.session_state:
            st.session_state.tool_history = []
        if 'messages' not in st.session_state:
            st.session_state.messages = [{"role": "system", "content": "You are smart music AI Assistant. You can find musics that users want. 并给出推荐和介绍"}]
