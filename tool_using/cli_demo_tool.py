...
tools = [{'name': 'music', 'description': '给用户推荐音乐', 'parameters': {'type': 'object', 'properties': {'number': {'description': '推荐的歌曲数量，阿拉伯数字'},
'artist': {'description': '歌手/艺术家名字'}, 
'name': {'description': '歌曲名'}, 
'type': {'description': '歌曲风格'}, 
'required': []}}}, 
...
{'type': 'object', 'properties': {'text': {'description': '需要转换成语音的文本'}, 'voice': {'description': '要使用的语音类型（男声、女声等）'}, 'speed': {'description': '语音的速度（快、中等、慢等）'}}, 'required': []}},
 {'name': '/image_resizer', 'description': '调整图片的大小和尺寸', 'parameters': {'type': 'object', 'properties': {'image_file': {'description': '需要调整大小的图片文件'}, 'width': {'description': '需要调整的宽度值'}, 'height': {'description': '需要调整的高度值'}}, 'required': []}},
